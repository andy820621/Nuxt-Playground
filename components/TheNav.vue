...
<template>
  <nav px4 py3 text-lg border="b base" flex="~ gap-1 items-center">
    ...
    <button v-if="play.mountedGuide?.solutions" @click="play.mountGuide(play.mountedGuide, !play.showingSolution)">
      Toggle Solution
    </button>
        ...
        <div px5 py4 grid="~ gap-y-3 gap-x-2 cols-[max-content_1fr] items-center">
          ...
          <NuxtLink :to="`${repo}/commit/${runtime.public.gitSha}`" target="_blank" title="View on GitHub">
            <time :datetime="buildTime.toISOString()" :title="buildTime.toLocaleString()">
              Built {{ timeAgo }} (<code>{{ (runtime.public.gitSha as string).slice(0, 5) }}</code>)
            </time>
          </NuxtLink>
        </div>
      </template>
    ...
    <NuxtLink
      ...
      duration-200
      hover="bg-active"
      href="https://github.com/andy820621/Nuxt-Playground" target="_blank"
    >
